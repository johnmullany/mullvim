...
return {
      ...
      on_attach = function(_, bufnr)
        ...
        if vim.fn.expand("%:h") == "dnd" then
          ...
          vim.keymap.set(
            "n",
            "<A-j>",
            [[:lua mullvim.dnd.move_to("previous")<cr>]],
            opts { desc = "Previous D&D note" }
          )
          vim.keymap.set(
            "n",
            "<A-k>",
            [[:lua mullvim.dnd.move_to("next")<cr>]],
  ...
  { "farmergreg/vim-lastplace", event = { "BufReadPre", "BufNewFile" } },
  {
    ...
    opts = {
      ...
      bgImage = "/Users/john/Downloads/luke-chesser-pJadQetzTkI-unsplash.jpg",
      shadowBlurRadius = 0,
  ...
  { "tpope/vim-surround", event = { "BufReadPost", "BufNewFile" } },
  {
    ...
    config = function()
      ...
      vim.g.mull_term_auto_close = true

      vim.g["test#custom_strategies"] = {
        mull = function(cmd)
          local winnr = vim.fn.winnr()
          require("mullvim.term").open(cmd, winnr, vim_notify_notfier)
        end,
      }
      vim.g["test#strategy"] = "mull"
    end,
  },
  {
    "lewis6991/gitsigns.nvim",
